...
import { CategoryName, ExtractResult, Priority, TaskDraft } from '../domain/types';
...
function detectIntent(text: string) {
  if (/(끝|완료|했어|다\s?했|클리어)/.test(text)) return 'complete_quest' as const;
  if (/(건너|패스|스킵)/.test(text)) return 'skip_quest' as const;
  if (/(취소|빼줘|지워|삭제)/.test(text)) return 'cancel' as const;
  if (/(남았|뭐\s?하|뭐\s?남|보여줘|목록|뭐가)/.test(text)) return 'query' as const;
  if (/(다음|넥스트)/.test(text)) return 'next_quest' as const;
  return 'add_schedule' as const;
}

function replyFor(intent: string): string {
  switch (intent) {
    case 'complete_quest':
...
function cleanTitle(clause: string): string {
  ...
  t = t.replace(/(해야\s?지|해야\s?해|해줄래|해주라|해줘|할\s?거야|할게|할래|하자|하기|했어|해야|가고|가야|갈게|갔다|줘)/g, ' ');
