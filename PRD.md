...
발화(텍스트/음성)
  ...
  → Copilot SDK (chat.completions, Structured Output)
       └ 실패/키 없음 → 휴리스틱 파서(parseUtterance)로 폴백
...
### 7.3 폴백 전략 (무중단 보장)
- 서버에 `COPILOT_API_KEY`가 없거나 호출 실패 → 동일 인터페이스의 **휴리스틱 한국어 파서**가 응답.
...
| LLM | **Copilot SDK**(서버 전용, Structured Output) + 휴리스틱 폴백 |
...
- **접근성**: 의미 있는 `aria-label`, 키보드 입력 경로, `prefers-reduced-motion`.
- **보안**: LLM 키 서버 전용, `/api/extract` 입력 검증(과길이 차단), 비밀값(.env) 미배포.
- **복원력**: LLM 장애 시 휴리스틱 폴백으로 항상 응답.
...
| 평가 항목 | 충족 근거 |
|---|---|
| 동작하는 결과물 | SSR 단일 노드로 즉시 구동, Azure 배포 |
| 아이디어/차별성 | 음성→온톨로지→게임화 전 과정의 **라이브 시각화** |
| AI 활용 | Copilot SDK Structured Output + 의도분류 7종, 실패 시 폴백 |
| 기술 완성도 | Angular 21 Zoneless/Signals, SOLID Port/Adapter, 7개 signal store |
| UX/완성도 | 페르소나 톤, 진행 인디케이터, 온보딩, 8종 기능 런처, 소셜 |
| 투명성/신뢰 | SDK 콘솔에 실제 요청/응답·토큰·지연·LIVE/FALLBACK 노출 |
| 안정성 | 무키/장애 시 휴리스틱 폴백으로 무중단 데모 |
